...
namespace Nop.Web.Controllers
{
    public class LimousineController : BaseNhaXeController
    {
        ...
        public ActionResult _DatVe(int DatVeId, int ChuyenDiId,int SoDoGheId)
        {
            if (this.CheckNoAccessIntoNhaXe(_workContext, _permissionService, StandardPermissionProvider.CVHoatDongBanVe))
                return AccessDeniedView();
            ...
            else
            {
                ...
                var giatiens = _limousinebanveService.GetAllHanhTrinhLoaiXeGiaVe(chuyendi.HanhTrinhId, chuyendi.lichtrinhloaixe.LoaiXeId).Where(c => c.SoDoGheId == SoDoGheId).ToList();
                if (giatiens.Count > 0)
                    model.GiaTien = giatiens.FirstOrDefault().GiaVe;
                else
                    model.GiaTien = chuyendi.lichtrinhloaixe.GiaVe;
            }
        ...
        public ActionResult _DatVe(DatVeModel model)
        {
            ...
            var chuyendi = _limousinebanveService.GetChuyenDiById(model.ChuyenDiId);
            //if (ngaytao.AddHours(-1) > chuyendi.NgayDiThuc)
        ...
        public ActionResult QuanLyChuyen()
        {
            ...
            model.KhuVucId = (int)ENKhuVuc.QUANG_NINH;
        ...
        public ActionResult _TabChuyenDi(int HanhTrinhId, int LichTrinhId,int KhungGioId, DateTime NgayDi,string ThongTinKhachHang)
        {
            ...
            var chuyendis = _limousinebanveService.GetAllChuyenDi(_workContext.NhaXeId, HanhTrinhId, 0, ENKhungGio.All, NgayDi, "", ThongTinKhachHang);
            if(chuyendis.Count==0)
            {
                ...
                if(listHanhTrinh.Contains(HanhTrinhId))
                {
                    ...
                    {
                        var _thoigiandi = NgayDi.Date.AddHours(item.ThoiGianDi.Hour).AddMinutes(item.ThoiGianDi.Minute);
                        var chuyendi = new ChuyenDi();
                        chuyendi.HanhTrinhId = HanhTrinhId;
                        chuyendi.LichTrinhId = item.Id;
                        ...
                        var lichtrinhloaixe = _limousinebanveService.GetLichTrinhLoaiXe(HanhTrinhId, item.Id, (int)ENLoaiXe.XE_9_CHO);
