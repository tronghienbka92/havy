...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ExportManager : IExportManager
    {
        ...
        public virtual void ExportLuotTrongNgayToXlsx(Stream stream, List<XeDiTrongNgay> DoanhThuXes, DateTime NgayDi)
        {
            if (stream == null)
                throw new ArgumentNullException("stream");
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                worksheet.Cells["E2:H2"].Merge = true;
                worksheet.Cells["E2"].Value = "Ngày đi " + NgayDi.ToString("dd/MM/yyyy");
                worksheet.Cells["E2"].Style.Font.Bold = true;
        ...
        public virtual void ExportDTTongHopToXlsx(Stream stream, List<ListDoanhThuTongHop> DTTongHop, DateTime TuNgay, DateTime DenNgay)
        {
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                worksheet.Cells["B4"].Value = "QUẢNG NINH ";
                ...
                foreach (var item in DTTongHop)
                {
                    ...
                    worksheet.Cells[row, col].Value = item.DTQuangNinh.TongLuot;
                    col++;
                    worksheet.Cells[row, col].Value = item.DTQuangNinh.SoKhach;
                    col++;
                    worksheet.Cells[row, col].Value = item.DTQuangNinh.ThanhTien.ToString("###,###");
                    col++;




                    decimal Tong = item.DTQuangNinh.ThanhTien;
                ...
                worksheet.Cells[row, coltong].Value = DTTongHop.Sum(c => c.DTQuangNinh.TongLuot);
                coltong++;
                worksheet.Cells[row, coltong].Value = DTTongHop.Sum(c => c.DTQuangNinh.SoKhach);
                coltong++;
                worksheet.Cells[row, coltong].Value = DTTongHop.Sum(c => c.DTQuangNinh.ThanhTien).ToString("###,###");
                coltong++;
