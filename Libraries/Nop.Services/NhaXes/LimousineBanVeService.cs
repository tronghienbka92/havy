...
namespace Nop.Services.NhaXes
{
    public class LimousineBanVeService : ILimousineBanVeService
    {
        ...
        public virtual ListDoanhThuTongHop GetDoanhThuTongHopTheoNgay(DateTime NgayDi,int NhaXeId)
        {
            ...
            var QNHanhtrinhids = _hanhtrinhkhuvucRepository.Table.Where(c => c.KhuVucId == (int)ENKhuVuc.QUANG_NINH).Select(c => c.HanhTrinhId);

            //get chuyendi cua tung khu vuc
            var chuyendiQN = query.Where(c => QNHanhtrinhids.Contains(c.HanhTrinhId)).Select(c => c.Id).ToArray();

            //doanh thu quang ninh
            item.DTQuangNinh = new DoanhThuTongHop();
            var datvesnb = _datveRepository.Table.Where(c => chuyendiQN.Contains(c.ChuyenDiId.Value) && (c.TrangThaiId == (int)ENTrangThaiDatVe.DA_DI || c.TrangThaiId == (int)ENTrangThaiDatVe.DA_XEP_CHO));
            item.DTQuangNinh.TongLuot = datvesnb.Select(c => c.ChuyenDiId).Distinct().Count();
            item.DTQuangNinh.SoKhach = datvesnb.Count();
            item.DTQuangNinh.ThanhTien = 0;
            if (datvesnb.Count() > 0)
            {
                item.DTQuangNinh.ThanhTien = datvesnb.Sum(c => c.GiaTien);
            }           
            return item;
        }
        /*
